...
class TSPSolver:
	...
	def branchAndBound( self, time_allowance=60.0 ):
		...
		for i in range(len(cities)):
			soln = []
			oCity = cities[i]
			firstCity = matrixSolver.matrixSolver(cities, None, None, None, oCity)
			soln.append(oCity)
			total = 1
			# create initial state
			# insert first state in heap
			heapq.heappush(S, firstCity)
			...
			while len(S) != 0:
				...
				for i in curr.unvisited:
					# if the cost of travelling to that city (computed in the matrixSolver) is less than bssf
					testingCity = matrixSolver.matrixSolver(cities, curr.fromCity, i, curr, firstCity)
					total += 1
					if testingCity.cost < bssf:
						heapq.heappush(S, testingCity)
						soln.append(i)
				...
				if len(curr.unvisited) == 0:
					...
					backToFirst = matrixSolver.matrixSolver(cities, curr.fromCity, oCity, curr, oCity)
					total += 1
					# update bssf
					tN = time.time()
					if backToFirst.cost <= bssf:
						bssf = backToFirst.cost
						count += 1
